...
export function getPackagesToInstallFromArgv(argv) {
    return argv.map(arg => {
...
export async function installPackage(packageToInstall, installPath) {
...
export async function installPackages(packagesToInstall, installPath) {
    console.log('💾 Downloading packages...');
    return Promise.all(packagesToInstall.map(pkg => installPackage(pkg, installPath)));
}
export async function importPackageIntoContext(context, installedPackage) {
...
export async function importPackagesIntoContext(context, packages) {
...
export function isValidJsVariableName(name) {
    if (JS_RESERVED_WORDS.includes(name)) {
        return false;
    }
    return /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(name);
}
export function validatePackageNames(newPackages, alreadyInstalled = []) {
...
export function showReplHeader(packages) {
    console.log('');
    console.log(`Welcome to Node.js ${process.version}.`);
    console.log(`Type ".help" for more information.`);
    listPackages(packages);
}
export async function importAdditionalPackagesViaCommand({ args, replServer, installedPackages, installPath, }) {
    try {
        // Parse arguments
...
export function setupReplServer(replServer, { installPath, installedPackages, }) {
...
export function listPackages(packages, replServer) {
    console.log('');
